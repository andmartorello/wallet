...
class HelperApplicationGUI:
    def create_window(self, title, geometry="400x300", resizable=(False, False), modal=False):
        window = tk.Toplevel(self.root)
        window.title(title)
        window.geometry(geometry)
        window.resizable(*resizable)
        if modal:
            window.transient(self.root)  # Collega il dialogo alla finestra principale
            window.grab_set()  
        return window

    def create_treeview(self, parent, columns, height=15, layout="pack", **kwargs):
        frame = self.create_frame(parent, layout=layout)
        tree = ttk.Treeview(frame, columns=columns, show="headings", height=height, **kwargs)
        for col in columns:
    ...
    def create_widget(self, widget_class, parent, layout="pack", **kwargs):
        default_text = kwargs.pop("default_text", None)  # Valore predefinito per .insert()
        state = kwargs.pop("state", None)  # Stato del widget
        ...
        sticky = kwargs.pop("sticky", tk.W)

        widget = widget_class(parent, **kwargs)

        # Inserisci testo predefinito, se applicabile
        if default_text is not None and hasattr(widget, "insert"):
            widget.insert(0, default_text)

        # Configura stato, se specificato
    ...
    def create_combobox(self, parent, values, default_text=None, layout="pack", **kwargs):
        combobox = self.create_widget(ttk.Combobox, parent, values=values, layout=layout, **kwargs)
        if default_text:
            combobox.set(default_text)
        return combobox

    def create_frame(self, parent, layout="pack", **kwargs):
        frame = ttk.Frame(parent)
        if layout == "pack":
            frame.pack(fill=kwargs.pop("fill", tk.BOTH), expand=kwargs.pop("expand", True), padx=kwargs.pop("padx", 10), pady=kwargs.pop("pady", 10))
        elif layout == "grid":
            frame.grid(row=kwargs.pop("row", 0), column=kwargs.pop("column", 0), padx=kwargs.pop("padx", 10), pady=kwargs.pop("pady", 10), sticky=kwargs.pop("sticky", tk.W))
        return frame
...
class ApplicationGUI:
    ...
    def create_summary_tab(self):
        ...
        self.two_columns_frame = ttk.Frame(self.summary_frame)
        self.two_columns_frame.pack(fill=tk.BOTH, expand=True)

        self.percentuali_frame = ttk.Labelframe(self.two_columns_frame, text="Percentuali Portafoglio", padding=(10, 10))
        self.percentuali_frame.pack(side=tk.LEFT, fill=tk.BOTH, expand=True, padx=10, pady=10)

        self.altcoin_detail_frame = ttk.Labelframe(self.two_columns_frame, text="Dettaglio Altcoin", padding=(10, 10))
        self.altcoin_detail_frame.pack(side=tk.LEFT, fill=tk.BOTH, expand=True, padx=10, pady=10)
        ...
        self.altcoin_canvas = tk.Canvas(self.altcoin_detail_frame)
        self.altcoin_scrollbar = ttk.Scrollbar(self.altcoin_detail_frame, orient="vertical", command=self.altcoin_canvas.yview)
        self.altcoin_detail_content = ttk.Frame(self.altcoin_canvas)

        self.altcoin_detail_content.bind(
            "<Configure>",
            lambda e: self.altcoin_canvas.configure(
                scrollregion=self.altcoin_canvas.bbox("all")
            )
        )

        self.altcoin_canvas.create_window((0, 0), window=self.altcoin_detail_content, anchor="nw")
        self.altcoin_canvas.configure(yscrollcommand=self.altcoin_scrollbar.set)

        self.altcoin_canvas.pack(side=tk.LEFT, fill=tk.BOTH, expand=True)
        self.altcoin_scrollbar.pack(side=tk.RIGHT, fill=tk.Y)
    ...
    def display_summary(self):
        # Pulizia del contenuto esistente
        for widget in self.percentuali_content.winfo_children():
            widget.destroy()
        for widget in self.altcoin_detail_content.winfo_children():
            widget.destroy()

        current_crypto_prices = self.data_manager.get_current_crypto_prices()
        percentuali_target = self.data_manager.percentuali_target

        # Leggiamo il dizionario altcoin_details, se non esiste vuoto
        altcoin_details_target = percentuali_target.get("altcoin_details", {})
        ...
        def create_progressbar(label_text, value, target_value, parent_frame=None):
            if parent_frame is None:
                parent_frame = self.percentuali_content
            ...
            # Riduciamo la lunghezza a 225 (75% di 300)
            progress = ttk.Progressbar(frame, style=progress_style, orient='horizontal', length=225, mode='determinate')
        ...
        if total_portfolio_value > 0:
            ...
            # Tutto ciò che non è BTC, ETH, USDT o ETF_ è altcoin
            for currency, balance in self.balances.items():
                if currency.startswith('ETF_') or currency in ['USDT', 'BTC', 'ETH']:
                    continue
                current_price_eur = current_crypto_prices.get(self.data_manager.crypto_mapping.get(f"{currency}/USDT", ''), {}).get('eur', 0)
                altcoins_dict[currency] = balance * current_price_eur
            ...
            btc_percent = (total_btc_value / total_portfolio_value) * 100 if total_portfolio_value > 0 else 0
            eth_percent = (total_eth_value / total_portfolio_value) * 100 if total_portfolio_value > 0 else 0
            altcoins_percent = (total_altcoins_value / total_portfolio_value) * 100 if total_portfolio_value > 0 else 0
            ...
            # SEZIONE DETTAGLIO ALTCOIN
            if total_altcoins_value > 0:
                # Ogni altcoin è una percentuale del totale altcoin
                for alt_name, alt_value in altcoins_dict.items():
                    alt_relative_percent = (alt_value / total_altcoins_value) * 100
                    # Se l'altcoin è nel dizionario altcoin_details, usiamo quel valore, altrimenti 0
                    alt_target = altcoin_details_target.get(alt_name, 0)
                    create_progressbar(f"{alt_name}: {alt_value:,.2f} EUR", alt_relative_percent, alt_target, parent_frame=self.altcoin_detail_content)
            else:
                ttk.Label(self.altcoin_detail_content, text="Nessuna altcoin presente.", font=("Arial", 10, "bold")).pack(pady=5)
